...
function applySort() {
    // 1. EXIT EARLY IF THERE NO ITEMS TO SORT
    ...
    arrRecords.sort((a, b) => {
        ...
        // 4. INVERT THE RESULT IF THE USER CHOOSE DESCENDING(Z-A)
...
// 1. RUN THIS FUNCTION WHENEVER U CHANGE DATA(ADD, EDIT, DELETE)
function save() {
...
function loadOnPageStart() {
    ...
    // CHECKS IF LOCALDATA ACTUALLY HAS DATA BEFORE PARST
    if (storedData) {
        arrRecords = JSON.parse(storedData);
        console.log("Restored records from storage:", arrRecords);
    } else {
        arrRecords = []; // FALLBACK TO AN EMPTY ARRAY IF THE STORAGE IS CLEAR
    }
A
...
// EXECUTE THE LOAD FUNCTION RIGHT AWAY WHEN THE SCRIPT EXECUTE
loadOnPageStart();